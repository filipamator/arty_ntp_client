...
architecture rtl of ntp_packet is

signal packet       : t_packet;
type t_state is (ST_IDLE,ST_PKG,ST_IP4CHECKSUM,ST_UDPCHECKSUM,ST_STORE_CHKSM,ST_RUN);
...
--                packet(88) <= x"FF"; 
 --               packet(89) <= x"FF"; 
            ...
            when ST_RUN => 
                ...
                if (counter=packet'length) then
                    ST <= ST_IDLE;
                    counter <= 0;
                else
                    if (nibble='0') then
                        o_ntp_nibble <= packet(counter)(3 downto 0);
                        nibble <= '1';
                    else
                        o_ntp_nibble <= packet(counter)(7 downto 4);
                        nibble <= '0';
                        counter <= counter + 1; 
                    end if;
                end if;
